...
export default function Preloader() {
  ...
  useEffect(() => {
    const splitedPreloaderText = preloaderTextRef.current
      ? new SplitType(preloaderTextRef.current)
      : null;
    const LogoHeader = document.querySelector("#LogoHeader");
    const LogoheaderBoundingRect = LogoHeader
      ? LogoHeader.getBoundingClientRect()
      : null;
  ...
  return (
    ...
    >
      <div ref={preloaderTextRef} className="text-4xl text-gray-600">
        Arthur Guilet.
      </div>
    </div>
  );
