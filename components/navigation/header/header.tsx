    ...
    function handleMenu() {
        ...
        if(isMenuOpen) {
            for(let i = 0; i < hamburgerSpan.length; i++) {
                gsap.to(hamburgerSpan[i], {duration: 0.5, rotate: 0, delay: i * 0.1, onComplete: () => {
                    gsap.to(hamburger, {duration: 0.5, rotate: 0})
                }})
            }

            for(let i = 0; i < list.length; i++) {
                gsap.to(list[i], {duration: 0.5, opacity: 0, delay: i * 0.1, onComplete: () => {
                    gsap.to('.hidden_menu', {duration: 0.5, top: '-100vh'})
                }})}

        } else {
            gsap.to(hamburger, {duration: 0.5, rotate: 90, onComplete: () => {
                for(let i = 0; i < hamburgerSpan.length; i++) {
                    gsap.to(hamburgerSpan[i], {duration: 0.5, rotate: 90, delay: i * 0.1})
                }
            }})

            gsap.to('.hidden_menu', {duration: 0.5, top: '0vh', onComplete: () => {
                for(let i = 0; i < list.length; i++) {
    ...
    useEffect(() => {
        ...
        if(!isSubMenuOpen) {
            ...
            for(let i = 0; i < submenu_mobile.length; i++) {
                gsap.to(submenu_mobile[i], {duration: 0.5, opacity: 0, delay: i * 0.1, onComplete: () => {
                    gsap.to(submenu_mobile[i], {duration: 0.5, display: 'none',  onComplete: () => {
            ...
            gsap.to(submenu, {duration: 0.5, display: 'block', onComplete: () => {
                gsap.to(submenu, {duration: 0.5, opacity: 1})
            }})

                gsap.to(chevron, {duration: 0.5, rotate: -180, onComplete: () => {
                    for(let i = 0; i < submenu_mobile.length; i++) {
                        gsap.to(submenu_mobile[i], {duration: 0.5, display: 'flex', delay: i * 0.1, onComplete: () => {
                ...
                ">
                    ...
                    <li className='hidden md:flex md:max-w-xl items-center justify-between w-1/2'>
                        {menu.map((item, index) => {
                            if (item.subMenu) {
                                return (
                                    <div key={index} ref={SubmenuRef} onClick={() => {setIsSubMenuOpen(!isSubMenuOpen)}} className='desk_navlinks desk_navlinks_withsubmenu cursor-pointer'>
